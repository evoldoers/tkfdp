...
bioRxiv preprint ID: **BIORXIV/2026/725674** (awaiting screening as of 2026-05-16).

## GitHub release URLs (parameterised)

The reproduction recipe (Section 4 below) uses these placeholder
variables; substitute your own owner / repo name when forking:

```bash
RELEASE_OWNER=evoldoers   # owner of the canonical drop
RELEASE_REPO=tkfdp        # repo name
```
...
`s3://tkf-mixdom-gpu-<AWS_ACCOUNT>/qprime-cache/` (manifest at
...
- `s3://tkf-mixdom-gpu-<AWS_ACCOUNT>/qprime-cache/manifest_LATEST.json` — pointer to the most recent stash; current contents have 7 methods (tkf92, mixdom_d3f1, tkf92_K20, cherryml_C20, mafft, muscle, infinite_phmm_mcmc_K4_coupled_RE) covering 120 / 22 / 98 families respectively.
...
aws --profile tkf-gpu s3 cp \
    s3://tkf-mixdom-gpu-<AWS_ACCOUNT>/mixdom-checkpoints/svi_bw_d3f1_postfix_best_val.npz \
    ~/tkf-mixdom/python/pfam/svi_bw_d3f1_postfix_best_val.npz
aws --profile tkf-gpu s3 cp \
    s3://tkf-mixdom-gpu-<AWS_ACCOUNT>/mixdom-checkpoints/tkf92_mixture_K20_train.npz \
    ~/tkf-mixdom/python/pfam/tkf92_mixture_K20_train.npz
aws --profile tkf-gpu s3 cp \
    s3://tkf-mixdom-gpu-<AWS_ACCOUNT>/mixdom-checkpoints/cherryml_mixture_C20_n5000.npz \
