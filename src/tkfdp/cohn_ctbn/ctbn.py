...
# tkf-dp adaptation: evolsnake's N=2 unit tests converge inside diffrax's default
# 4096-step cap, but the 400-state (N=20) two-site case has stiffer rho/mu ODEs on
# off-diagonal (substitution) endpoint pairs. Raise the cap (4x) so Cohn's mean-field
# is given every chance to converge before we compare its tightness/cost. Keep it
# modest: SaveAt(dense=True) preallocates buffers of length max_steps per solve, so an
# astronomically large cap OOMs XLA compilation. (Only change vs the verbatim evolsnake
# copy; see PROVENANCE.md.)
CTBN_MAX_STEPS = 1 << 14   # 16384
...
def q_bar_cond (i, nbr_idx, nbr_mask, params, mu):
    ...
    exp_2J = jnp.exp (2 * J)  # (y_i,x_{nbr_k})
    mu_exp_2J = jnp.einsum ('kx,yx->ky', mu[nbr_idx[i]], exp_2J)  # (k,y_i)
    mu_exp_2JC = mu_exp_2J[None,:,:] ** nonself_nbr_mask[:,:,None]  # (j,k,y_i)
    S = S * offdiag_mask(N)
    return S[None,None,:,:] * jnp.exp(h)[None,None,None,:] * jnp.exp(2*cond_energy)[:,:,None,:] * product(mu_exp_2JC,axis=-2)[:,None,None,:]  # (j,x_{nbr_j},x_i,y_i)
...
def psi (i, nbr_idx, nbr_mask, params, mu, rho):
    M = nbr_idx.shape[-1]
    nu = nbr_idx[i]  # (M,) component indices of i's neighbors
    # For each j in nu, find slot s such that nbr_idx[j, s] = i, so we can pull j's q_bar/q_tilde conditioned on x_i
    i_slot = jnp.argmax (nbr_idx[nu] == i, axis=-1)  # (M,)
    qbar_per_j = jax.vmap (lambda j: q_bar_cond(j, nbr_idx, nbr_mask, params, mu))(nu)  # (j,slot,x_i,x_j,y_j)
    qtilde_per_j = jax.vmap (lambda j: q_tilde_cond(j, nbr_idx, nbr_mask, params, mu))(nu)
    qbar_at_i = jax.vmap (lambda j, s: qbar_per_j[j, s])(jnp.arange(M), i_slot)  # (j,x_i,x_j,y_j)
    qtilde_at_i = jax.vmap (lambda j, s: qtilde_per_j[j, s])(jnp.arange(M), i_slot)
    log_qtilde_at_i = safe_log (jnp.where (qtilde_at_i < 0, 1, qtilde_at_i))
    gammas = gamma (nu, nbr_idx, nbr_mask, params, mu, rho)  # (j,x_j,y_j)
    return -jnp.einsum('jy,jxyz,j->x',mu[nu],qbar_at_i,nbr_mask[i]) + jnp.einsum('jyz,jxyz,j->x',gammas,log_qtilde_at_i,nbr_mask[i])
...
def solve_F (seq_mask, nbr_idx, nbr_mask, params, mu_solns, rho_solns, T, rtol=1e-3, atol=1e-6):
    ...
    F_soln = diffrax.diffeqsolve (terms=term, solver=solver, t0=0, t1=T, dt0=None, y0=0,
                                  ...
                                  stepsize_controller = controller, max_steps=CTBN_MAX_STEPS)
    return F_soln.ys[-1]
...
def ctbn_variational_log_cond (prng, xs, ys, seq_mask, nbr_idx, nbr_mask, params, T, min_inc=1e-3, max_updates=4096, rtol=1e-3, atol=1e-6):
    # tkf-dp: rtol/atol added to the signature and threaded to every solve so the ODE
    # tolerance (which governs the accuracy of the log-singular F integral) is tunable.
    K = nbr_idx.shape[0]
    N = params['S'].shape[0]
    ...
    rho_solns = [solve_rho (i, rho_T[i,:], seq_mask, nbr_idx, nbr_mask, params, init_mu_solns, init_rho_solns, T, rtol=rtol, atol=atol) for i in range(K)]
    mu_solns = [solve_mu (i, mu_0[i,:], seq_mask, nbr_idx, nbr_mask, params, init_mu_solns, rho_solns, T, rtol=rtol, atol=atol) for i in range(K)]
    ...
    def score_fun (outer_state):
        ...
        F = solve_F (seq_mask, nbr_idx, nbr_mask, params, mu_solns, rho_solns, T, rtol=rtol, atol=atol)
        return F
...
#        jax.debug.print("order={order}",order=order)
    ...
    def loop_body_fun (inner_state, i):
        ...
        new_rho_i = solve_rho (i, rho_T[i,:], seq_mask, nbr_idx, nbr_mask, params, mu_solns, rho_solns, T, rtol=rtol, atol=atol)
        rho_solns = replace_nth (rho_solns, i, new_rho_i)
        new_mu_i = solve_mu (i, mu_0[i,:], seq_mask, nbr_idx, nbr_mask, params, mu_solns, rho_solns, T, rtol=rtol, atol=atol)
