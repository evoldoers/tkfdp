...
def assign_times(forest, rng=None, label_order='spatial'):
    ...
    if label_order == 'temporal':
        ...
        raise ValueError(
            f"label_order must be 'spatial' or 'temporal', got {label_order!r}")
...
def _draw_lottery_row(ax, forest, n_events, lottery_seed):
    """Draw a row of lottery-ticket cartoons below each column.  Two
    distant alive columns happen to share an ID (the speed-dating
    match); arc + heart annotate them as a Potts-coupled pair."""
    ...
    rng.shuffle(pool)
    ids = pool[:max_col]    # one per column, all distinct

    alive_cols = [n['col'] for n in nodes_by_col if n['type'] in 'MI']
    # Prefer two well-separated alive columns; fall back to any.
    if len(alive_cols) >= 2:
        best = (None, None, -1)
    ...
    ids[match_j - 1] = ids[match_i - 1]   # force the collision

    # Lottery-ticket palette: all tickets a pale pink-red; the matched
    # pair gets a brighter, more saturated red to call out the
    # speed-dating collision visually.
    PALE_RED = mcolors.hsv_to_rgb([0.00, 0.18, 1.00])    # very light pink
    BRIGHT_RED = mcolors.hsv_to_rgb([0.00, 0.55, 1.00])  # vivid coral red

    def ticket_color(col_idx_1based):
        if col_idx_1based in (match_i, match_j):
            return BRIGHT_RED
        return PALE_RED

    # Tickets sit just below the T line.
    y_top = n_events + 1
    y_t0 = y_top + 0.5     # top of ticket
    y_t1 = y_top + 1.6     # bottom of ticket
    tw = 0.78
    th = y_t1 - y_t0
    ...
    # Speed-dating match: arc + heart between the two columns sharing a ticket.
    if match_i != match_j:
        y_arc_top = y_t1 + 0.15
        ...
        arc = PathPatch(Path(verts, codes), facecolor='none',
                         edgecolor='#C40233', linewidth=1.6, zorder=4)
        ax.add_patch(arc)
        x_mid = 0.5 * (match_i + match_j)
        y_mid = y_arc_peak - 0.05
        # Heart-shape "MATCH" badge: use Unicode heart with white-on-red label.
        ax.text(x_mid, y_mid, '♥ MATCH ♥',
                ...
                bbox=dict(boxstyle='round,pad=0.30,rounding_size=0.30',
                          ...
                          linewidth=0),
                zorder=6)
        bottom_extra = y_arc_peak + 0.5
    else:
        bottom_extra = y_t1 + 0.3
    return bottom_extra
...
def draw_figure(forest, n_events, output_path, lottery_seed=None):
    ...
    # Immortal link at column 0 (dashed).
    ...
    for n in nodes:
        ...
        # Horizontal branching arrow for insertions.
        if n['type'] in 'IG':
            ap = n.get('arrow_parent')
    ...
    # Optional lottery-ticket row + speed-dating MATCH arc.
    ...
    # x-tick labels: column types (M/I/D/G) along the top.
...
def main():
    ...
    p.add_argument('--lottery-seed', type=int, default=None,
                   help='If given, add a row of lottery-ticket cartoons '
                   'below the trajectory (one per column), each with a '
                   '3-digit "ID number" drawn pseudo-randomly. Two '
                   'distant alive columns are forced to share an ID '
                   '(the speed-dating "match") and joined by an arc '
                   'with a MATCH heart -- the CRP / Potts coupling '
                   'cartoon for two co-keyed sites.')
